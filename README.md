...
The SDK ships `DashXRCTAppDelegate` which handles permission requests, notification display, and universal link forwarding. Subclass it in your `AppDelegate.swift` and configure Firebase directly — Firebase must be imported and initialised in the app target because `DashXRCTAppDelegate` cannot import Firebase from inside a CocoaPod:

```swift
import DashXReactNative
import ReactAppDependencyProvider  // only needed for React Native New Architecture
import FirebaseCore
...
class AppDelegate: DashXRCTAppDelegate {
  ...
  override func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
    self.dependencyProvider = RCTAppDependencyProvider()  // required for React Native New Architecture
